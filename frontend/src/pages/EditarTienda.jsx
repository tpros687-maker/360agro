...
export default function EditarTienda() {
  ...
  return (
    <div className="bg-agro-midnight min-h-screen pt-32 pb-24 px-6 relative overflow-hidden">
      <div className="absolute top-0 left-0 w-[500px] h-[500px] bg-agro-teal/5 rounded-full blur-[150px] -ml-20 -mt-20 opacity-20"></div>
        ...
        <form onSubmit={handleSubmit} className="space-y-12">
          ...
          <section className="card-midnight p-10 bg-agro-charcoal/40 border-white/5 space-y-10">
              ...
              <div className="flex flex-col md:flex-row items-center gap-8">
                {logoActual && (
                  <div className="relative group">
                    <img src={`${BASE_URL}${logoActual}`} className="w-32 h-32 rounded-3xl object-contain bg-agro-midnight border border-white/10 p-4" alt="Logo" />
                ...
                <label className="flex-1 w-full h-32 border-2 border-dashed border-white/5 rounded-3xl flex flex-col items-center justify-center cursor-pointer hover:border-agro-teal/30 transition-all">
                  <span className="text-2xl mb-1">📁</span>
                  <span className="text-[9px] font-black text-white/20 uppercase tracking-widest">Subir Nuevo Logo</span>
                  <input type="file" onChange={handleNuevoLogo} className="hidden" />
                  {logoNuevo && <span className="text-agro-teal text-[8px] mt-2 font-black italic">{logoNuevo.name}</span>}
                </label>
              </div>
            </div>
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {fotosActuales.map((f, i) => (
                  <div key={i} className="relative group h-24 rounded-xl overflow-hidden border border-white/10">
                    <img src={`${BASE_URL}${f}`} className="w-full h-full object-cover grayscale group-hover:grayscale-0 transition-all" />
              ...
              <label className="block w-full py-6 border-2 border-dashed border-white/5 rounded-2xl text-center cursor-pointer hover:border-agro-teal/30 transition-all">
                <span className="text-[10px] font-black text-white/20 uppercase tracking-[0.2em]">➕ Añadir Assets Visuales</span>
