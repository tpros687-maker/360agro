...
export default function LoteDetalle() {
  ...
  const tokenPayload = (() => { try { const t = localStorage.getItem("token"); return t ? JSON.parse(atob(t.split(".")[1])) : null; } catch { return null; } })();
  const idLogueado = tokenPayload?.id || tokenPayload?._id || null;
  ...
  const handleContactarWhatsapp = async () => {
    try {
      ...
      if (!nroSaneado) {
        toast.error("El productor no tiene WhatsApp registrado");
        return;
      }
      await API.patch(`/lots/${id}/interaccion`, { tipo: 'whatsapp' });
      trackEvent('Lotes', 'whatsapp_click', lote.titulo);
      const texto = encodeURIComponent(`Hola ${lote.usuario?.nombre}, soy ${usuario?.nombre} y vi tu lote "${lote.titulo}" (${lote.cantidad} cabezas de ${lote.raza}, USD ${lote.precio?.toLocaleString()}) publicado en 360 Agro. Me interesa negociar, ¿podemos hablar?`);
      ...
      <div className="container mx-auto relative z-10 max-w-7xl">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
          ...
          <div className="lg:col-span-7 space-y-6">
            <div className="bg-surface-container-high border border-outline-variant/70 p-4 rounded-[2.5rem] shadow-2xl relative overflow-hidden">
          ...
          <div className="lg:col-span-5 flex flex-col gap-6">
            <section className="bg-surface-container-high border border-outline-variant/70 p-10 rounded-[3rem] shadow-2xl relative">
              <div className="flex flex-wrap gap-2 mb-8">
                ...
                {lote.documentoPropiedad && lote.numeroDicose && (
                  <span className="bg-primary/10 text-primary text-[9px] font-black px-4 py-1.5 rounded-full uppercase tracking-widest border border-primary/30 italic flex items-center gap-2">
                    <span className="material-symbols-outlined text-[12px]">verified_user</span> Verificado
                  </span>
                )}
                <span className={`text-[9px] font-black px-4 py-1.5 rounded-full uppercase tracking-widest border italic ${
                  lote.estado === "Vendido" ? "bg-red-500/10 text-red-400 border-red-500/30" :
                  lote.estado === "Reservado" ? "bg-yellow-500/10 text-yellow-400 border-yellow-500/30" :
              ...
              {/* VALUACIÓN */}
              <div className="bg-surface-container-lowest p-8 rounded-[2rem] border border-outline-variant/70 mb-8 shadow-inner relative group overflow-hidden">
              ...
              <div className="grid grid-cols-3 gap-3 mb-8">
                <div className="bg-surface-container-low/50 p-5 rounded-3xl border border-outline-variant/70 text-center group hover:bg-primary-container transition-all">
              ...
              <div className="bg-surface-container-lowest border border-outline-variant/70 rounded-2xl p-6 mb-8">
              ...
              {lote.numeroDicose && (
                <div className="bg-surface-container-lowest border border-outline-variant/70 rounded-2xl p-6 mb-8 relative overflow-hidden">
                  <div className="absolute top-0 right-0 p-4 opacity-5">
                    <span className="material-symbols-outlined text-6xl text-primary">verified</span>
                  </div>
                  <h4 className="text-[10px] font-bold text-primary uppercase tracking-[0.4em] mb-6 italic">Certificación Oficial</h4>

                  <div className="space-y-6 relative z-10">
                    <div>
                      <p className="text-[8px] text-on-surface-variant/60 font-bold uppercase tracking-widest mb-1">DICOSE / ID Registrada</p>
                      <p className="text-xl font-black text-primary italic tracking-widest">{lote.numeroDicose}</p>
                    </div>
                    ...
                    <div className="flex flex-wrap gap-4 pt-4 border-t border-outline-variant/10">
                      {lote.documentoPropiedad && (
                        <a
                          ...
                          target="_blank"
                          rel="noreferrer"
                          className="flex items-center gap-2 text-[9px] font-bold text-primary hover:text-white transition-colors border border-primary/30 px-4 py-2 rounded-xl bg-primary/10"
                        >
              ...
              <div className="bg-surface-container-lowest border border-outline-variant/70 rounded-2xl p-6">
                <div className="space-y-4">
                  {usuarioLogueado && idLogueado === lote.usuario?._id?.toString() ? (
                    <p className="text-center text-[10px] text-on-surface-variant/40 font-bold uppercase tracking-widest py-4 border border-outline-variant/10 rounded-2xl italic">
                      Este es tu lote
                    </p>
                  ) : usuarioLogueado ? (
                    <>
                      <button
                        ...
                        className="w-full py-5 bg-[#25D366]/10 border border-[#25D366]/30 text-[#25D366] rounded-full font-bold text-[11px] uppercase tracking-widest hover:bg-[#25D366] hover:text-white transition-all shadow-xl flex items-center justify-center gap-3"
                      >
                        <span className="material-symbols-outlined text-lg">chat</span> Negociar vía WhatsApp
                      </button>
                      <button
                        onClick={() => { setMostrarModal(true); setMensaje(`Hola ${lote.usuario?.nombre}, soy ${usuario?.nombre}. Vi tu lote "${lote.titulo}" publicado en 360 Agro y me interesa negociar.`); }}
                        className="w-full py-5 bg-surface-variant/20 text-on-surface border border-outline-variant/50 rounded-full font-bold text-[11px] uppercase tracking-widest hover:bg-surface-variant transition-all flex items-center justify-center gap-3"
                      >
            ...
            {/* CARD PRODUCTOR */}
            <div className="bg-surface-container-high border border-outline-variant/60 p-8 rounded-[2.5rem] flex items-center justify-between group shadow-xl">
              <div>
                ...
                <div className="flex items-center gap-2 mt-2">
                  <span className="w-2 h-2 bg-primary rounded-full animate-pulse shadow-xl"></span>
                  <span className="text-[9px] font-bold text-primary uppercase tracking-widest">Socio Estratégico {lote.usuario?.plan}</span>
                </div>
              </div>
              <div className="w-16 h-16 bg-surface-container-lowest rounded-2xl flex items-center justify-center text-3xl border border-outline-variant/30 shadow-inner group-hover:scale-110 transition-transform">
      ...
      {mostrarModal && (
        <div className="fixed inset-0 bg-background/80 backdrop-blur-xl flex items-center justify-center z-[200] p-6 animate-in fade-in duration-300">
          <div className="bg-surface-container-highest border border-outline-variant/60 rounded-[3rem] w-full max-w-lg p-10 relative shadow-2xl">
            ...
            <h2 className="text-2xl font-black text-on-surface italic uppercase mb-2 tracking-tighter">Propuesta Directa</h2>
            ...
            <button onClick={enviarMensaje} className="machined-gradient w-full mt-8 py-5 text-[11px] font-bold uppercase tracking-widest shadow-xl rounded-full text-on-tertiary-fixed italic">
              ENVIAR PROPUESTA <span className="material-symbols-outlined text-sm">arrow_forward</span>
            </button>
          </div>
        </div>
      )}
